...
readonly AUDIO_URL="https://github.com/emu-list/8mb/raw/refs/heads/main/Odyssey.zip"
readonly REPO_URL="https://github.com/robertkirkman/sm64ex-omm.git"

# Archivos locales
readonly AUDIO_ZIP="Odyssey.zip"
readonly AUDIO_DIR="Odyssey"
...
extract_audio_files() {
    log "Descargando archivos de audio de Mario Odyssey..."
    ...
    if unzip -q "$AUDIO_ZIP" -d .; then
        ...
        if [[ -d "$AUDIO_DIR" ]] && [[ -n "$(ls -A "$AUDIO_DIR" 2>/dev/null)" ]]; then
            ...
            log_success "Archivos de audio de Mario Odyssey movidos a $SAMPLE_DIR (reemplazando existentes)"
...
clean_build() {
    ...
    # Ahora descargar y mover los archivos de Mario Odyssey (el truco está aquí)
    log "Descargando archivos de audio de Mario Odyssey..."
...
setup_project() {
    ...
    if [[ -d "$PROJECT_DIR" ]]; then
        ...
        if $needs_clean_build; then
            ...
            # Extraer assets normalmente si no hay compilación previa y después mover audios de Mario Odyssey
            if [[ -f "./baserom.us.z64" ]]; then
                log "Extrayendo assets del juego..."
                if python extract_assets.py us >/dev/null 2>&1; then
                    log_success "Assets extraídos correctamente"
                    
                    # Descargar y mover archivos de Mario Odyssey después de extraer assets
                    log "Descargando archivos de audio de Mario Odyssey..."
                    extract_audio_files || log_warning "No se pudieron obtener archivos de audio"
                else
                    log_error "Error al extraer assets"
        ...
        if git clone --recursive "$REPO_URL" "$PROJECT_DIR" >/dev/null 2>&1; then
            ...
            # Extraer assets para nueva instalación y después mover audios de Mario Odyssey
            if [[ -f "./baserom.us.z64" ]]; then
                log "Extrayendo assets del juego..."
                if python extract_assets.py us >/dev/null 2>&1; then
                    log_success "Assets extraídos correctamente"
